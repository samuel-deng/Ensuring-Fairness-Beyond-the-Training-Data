...
\begin{algorithm}[H]
\DontPrintSemicolon
\KwInput{all the inputs}
\KwOutput{all the outputs}

$w_0(i) = 1/n$ for all $i \in [n]$\\
$h_0 \in \argmin_{h \in \HH_{\WW}} \frac{1}{n} \sum_{i=1}^n \ell(h(x_i,a_i), y_i)$\\
%$\qquad \text{s.t. } \delta^{w_0}_F(h) \le \epsilon$\\
\For{each time step $t \in [T]$}
{
	$w_t = w_{t-1} + \eta \nabla_w \ell(h_{t-1},w_{t-1})$\\
	$w_t = \Pi_{\WW}(w_t)$\\
	$h_t \in \argmin_{h \in \HH_{\WW}} \sum_{i=1}^n w_t(i) \ell(h(x_i,a_i),y_i)$\\
	%$\qquad \text{s.t. } \delta^{w_t}_F(h) \le \epsilon$
}
\caption{Meta-Algorithm}
\end{algorithm}

We first discretize the set of weights $\WW$ as follows. For each $i \in [n]$, consider the buckets $B_0 = [0,\delta)$, $B_{j+1} = [(1+\eps)^j \delta, (1+\eps)^{j+1}\delta)$ for $j=0,1,\ldots,M-1$ for $M = O(\log_{1+\eps}(1/\delta) )$. For any weight $w\in \WW$, we consider the weight $w'$. Here $w'_i$ is the upper-end point of the bucket containing $w_i$. Note that this guarantees that either $w_i\le \delta$ or $\frac{w'_i}{1+\eps} \le w_i \le w_i'$. Now we shwo that  fairness guarantee with respect to the weight $w'$ is sufficient to guarantee fairness with respect to the weight $w$. 
%
\begin{align*}
...
\begin{align}
&\min_{h \in \HH} \sum_{i=1}^n w_1(i) \ell(h(x_i,a_i),y_i)\label{eq:final-objective}\\
&\text{s.t.} \delta^w_F(h) \le \eps / 5\ \forall w \in N(\eps/5,\WW) \nonumber
\end{align}
We form the following Lagrangian.
\begin{align}
\min_{h \in \HH} \max_{\lambda \in \bbR^{\abs{N(\eps/5,\WW)}}_+, \norm{\lambda}_1 \le B} \sum_{i=1}^n w_1(i) \ell(h(x_i,a_i),y_i) + \sum_{w \in N(\eps/5,\WW)} \lambda_w (\delta^w_F(h) - \eps / 5)
\end{align}
