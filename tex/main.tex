...
\Else{
	\Return{$\lambda^{a,a'}_w = 0$ for all $a,a' \in \Ac$ and $w \in N(\eps/5,\WW)$.}
}
\caption{Best Response of the $\lambda$-player\label{algo:best-lambda}}
\end{algorithm}

\begin{theorem}
Algorithm \ref{algo:best-lambda} is an $\eps/5+\gamma$-approximate best response for the $\lambda$-player.
\end{theorem}
\begin{proof}
