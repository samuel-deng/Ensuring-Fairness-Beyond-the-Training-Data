...
class LambdaBestResponse:
    ...
    def best_response(self):
        ...
        for (a, a_p) in a_a_p:
            ...
            h_xi_ap = self.h_pred.copy()
            h_xi_ap[self.a_indices[a_p]] = 0
            pi_0 = cp.Parameter(nonneg=True)
            pi_1 = cp.Parameter(nonneg=True)

            # Constraints
            constraints = [
                cp.sum(w[self.a_indices[a]]) == 1/pi_0,
                cp.sum(w[self.a_indices[a_p]]) == 1/pi_1,
                cp.sum(w) == 1/pi_0 + 1/pi_1,
                0 <= w
            ]

            # Objective Function
            objective = cp.Maximize((pi_0 * (w @ h_xi_a)) - (pi_1 * (w @ h_xi_ap)))
            prob = cp.Problem(objective, constraints) 
